...
`needs_review`では、顧客へ「内容を確認のうえご案内します」と受付文を返し、元の発話、匿名化したuser ID、conversation ID、判定理由、検索候補、score、判定RevisionをSQLiteのreview queueへ保存する。実際に担当者へ転送できていない段階で「担当者へ転送しました」とは表示しない。受付文、通知先、回答期限はConfigurationで管理する。
