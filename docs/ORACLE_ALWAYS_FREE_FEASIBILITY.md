...
2026-09-22の実作業では、OCIサインアップ画面でChromeの自動入力や入力履歴を利用した際、動的フォームの表示が競合して白画面になる事象を確認した。シークレットモードを使い、ブラウザの自動入力を避けて手入力することで作成を完了した。これは本端末で確認した運用上の注意であり、Oracleの恒久仕様とはみなさない。再現時の詳細な回避手順は[OCI移管リハーサル](OCI_MIGRATION_REHEARSAL.md#ociアカウント作成時の実績メモ)に記載する。

管理者はスマートフォン用認証アプリによるMFAを設定できることを前提とする。納品先の管理者、認証端末、復旧手段を事前に決め、MFAのシード、QRコード、ワンタイムコード、復旧情報をリポジトリや納品物へ含めない。
