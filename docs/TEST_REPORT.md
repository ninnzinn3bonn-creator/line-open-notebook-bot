...
| OCI image固定 | PASS (MANIFEST) / REAL HOST PENDING | 固定digestのOpen Notebook、SurrealDB、Cloudflared、Caddyすべてに`linux/arm64` manifestが存在。OCI Arm64実起動は未実施 |
...
| 実店舗データ変換 | PASS (UNIT/CLI) | 公開FAQだけのKnowledge、質問表現ごとのGolden Dataset、入力SHA-256 manifestを生成。非公開FAQ除外と重複ID拒否を確認 |
| MMP暫定合否・差分 | READY / STORE APPROVAL PENDING | PoC証拠から品質、性能、同時利用、有人移管、重複、障害、復旧の暫定値とMMPまでの差分を作成 |
