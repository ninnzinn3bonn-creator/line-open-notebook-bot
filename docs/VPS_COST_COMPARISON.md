...
- Ubuntu Serverの最小構成を使用する。
- 4GBのSwapを設定する。
- Ollama、ローカルEmbedding、音声・Podcast生成を使用しない。
- Open Notebookのバックグラウンド処理とSurrealDB処理の並列数を1にする。
- 回帰テストRunnerはVPSへ常駐させず、開発PCから逐次実行する。
- Knowledgeの一括投入中はモデル回帰テストを同時実行しない。
- Dockerログにサイズ上限と世代数を設定する。
- ハードなコンテナメモリ上限で強制的に詰め込まず、ホスト全体のRSS、Swap、OOMを測る。

次をすべて満たした場合だけ、2GBをPoC提出候補として扱う。

- 全コンテナ起動後の定常メモリが1.4GB以下。
- 通常回答中の使用メモリが1.7GB以下。
- 代表Knowledge投入後もOOM Killとコンテナ再起動が0件。
- Golden Dataset 30〜50問を逐次実行して完走する。
- 24時間のアイドル・復帰試験でHealth Checkが継続して成功する。
- 通常時のSwap使用が512MB以下で、継続的なSwap I/Oが発生しない。

いずれかを満たさない場合は、構成の削減や非公式なOpen Notebook改造で対応せず、4GBへ変更する。KAGOYAでは2GBが月額上限770円、4GBが1,760円なので、2GBが成立すれば月990円削減できる。3GBは月額上限1,430円で4GBとの差が330円しかないため、公式最小要件を下回るリスクに対して削減効果が小さく、提出構成には選ばない。
