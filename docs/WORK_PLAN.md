...
8. [完了] LINE回答に「営業時間」「アクセス」「予約について」「担当者に相談」のクイックリプライを表示
9. [実装済み・公式設定待ち] LINE公式チャットを有人対応画面・通知経路として利用し、BridgeがLINE user ID単位のBot停止・対応完了後の復帰を管理する。チャットON、通知、手動返信、二重返信は実機確認する
...
6. 異なるLINEアカウント2つ以上で、会話と処理順が混ざらないことを確認

## Track 3: 実店舗情報の受入れ

1. [準備済み] `REAL_STORE_DATA_INTAKE.md`に従い、承認者、機密区分、版、適用日を記録して受領
2. 受領原本をGit管理外の保護領域へ保存し、秘密情報・個人情報を除いた作業用コピーを作成
3. [変換ツール実装済み・データ待ち] FAQ、料金、営業時間、キャンセル条件、連絡先を公開可FAQだけのKnowledgeへ変換し、店舗承認を得る
4. [生成ツール実装済み・データ待ち] 合成Datasetを実店舗由来Golden Datasetへ置換し、人間評価、Critical判定、Source整合性を確認
5. 有人案内文、電話番号、受付時間、通知先を確定
6. [手順書作成済み] クライアント担当者によるKnowledgeの登録・変更・削除・承認・ロールバック運用
